...
Live solar-wind forcing reuses HelioSat's NOAA SWPC RTSW object products (`json/rtsw/rtsw_mag_1m.json`, `json/rtsw/rtsw_wind_1m.json` and `json/rtsw/rtsw_ephemerides_1h.json`). The connector follows the per-record `active` designation instead of hardcoding a spacecraft, then applies the existing MRU propagation. These are preliminary near-real-time values and can be revised; they are never presented as the retrospective OMNI reference archive. The retired `products/solar-wind/*-{2-hour,7-day}.json` tables are not queried.
...
Future orbit context comes from CelesTrak GP data and SGP4/satellite.js. The implemented request is `https://celestrak.org/NORAD/elements/gp.php?GROUP=<GROUP>&FORMAT=TLE`, matching the documented uppercase query contract at <https://celestrak.org/NORAD/documentation/gp-data-formats.php>. Upstream resolution is limited to one cycle every two hours, coalesces concurrent requests, opens a two-hour negative circuit after failure, and can serve only a previously downloaded real catalog from the Next data cache or the ignored runtime directory `data/cache/celestrak/`; no mirror or synthetic catalog is substituted. A cold-cache failure returns HTTP 503 and leaves the forecast unavailable. Runtime cache files are not committed, but CelesTrak/Space-Track redistribution history is legally non-trivial, so public or commercial redistribution remains pending explicit review: <https://celestrak.org/norad/elements/notice.php>. The TLE representation cannot carry newly cataloged six-digit objects; complete future catalog coverage requires a separately validated OMM ingestion and propagation path.
